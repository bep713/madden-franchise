...
schemaGenerator.getExtraSchemas = () => {
    const filePath = path.join(__dirname$5, '../data/schemas/extra-schemas.json');
...
async function generateSchemaV2({ fileMap, extraSchemas }) {
    ...
    if (!extraSchemas || extraSchemas.length === 0) {
        ...
        try {
            const extraPath = path.join(__dirname$4, '../data/schemas/extra-schemas.json');
...
const slotsLookup = JSON.parse(fs.readFileSync(path.join(__dirname$3, '../data/lookup-files/slotsLookup.json'), 'utf8'));
const fieldLookup = JSON.parse(fs.readFileSync(path.join(__dirname$3, '../data/lookup-files/fieldLookup.json'), 'utf8'));
const enumLookup = JSON.parse(fs.readFileSync(path.join(__dirname$3, '../data/lookup-files/enumLookup.json'), 'utf8'));
...
class IsonProcessor {
  ...
  loadGameYearData() {
    ...
    if (!IsonProcessor.internedLookups[this.gameYear]) {
      const lookupFilePath = path.join(__dirname$2, `../data/interned-strings/${this.gameYear.toString()}/lookup.json`);
  ...
  loadGameYearDataStatic(gameYear) {
    if (!IsonProcessor.internedLookups[gameYear]) {
      const lookupFilePath = path.join(__dirname$2, `../data/interned-strings/${gameYear.toString()}/lookup.json`);
      if (fs.existsSync(lookupFilePath)) {
...
let dictionary = fs.readFileSync(path.join(__dirname$1, '../data/zstd-dicts/26/dict.bin'));

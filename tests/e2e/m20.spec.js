                ...
                describe('reads records', () => {
                    ...
                    it('reads records correctly', () => {
                        ...
                        expect(record._data).to.eql(
                            Buffer.from([
                                0x21, 0x0a, 0x00, 0x5d, 0x21, 0x0a, 0x08, 0xf6,
                                0x21, 0x0a, 0x02, 0xa0
                            ])
                        );
                        expect(record.Player0).to.eql(
                            '00100001000010100000000001011101'
                        );
                        expect(record.Player1).to.eql(
                            '00100001000010100000100011110110'
                        );
                        expect(record.Player2).to.eql(
                            '00100001000010100000001010100000'
                        );
                        expect(record.hexData).to.eql(
                            Buffer.from([
                                33, 10, 0, 93, 33, 10, 8, 246, 33, 10, 2, 160
                            ])
                        );
                    });
        ...
        describe('Player table', () => {
            ...
            describe('reads records that are passed in', () => {
                ...
                it('has expected offset table', () => {
                    ...
                    expect(offset201.name).to.equal(
                        'OriginalBlockSheddingRating'
                    );
                ...
                describe('records have expected values', () => {
                    it('first record', () => {
                        ...
                        expect(record.SeasonStats).to.equal(
                            '00111100010100100000000000000000'
                        );
                    ...
                    it('Marcus Maye', () => {
                        ...
                        expect(record.GameStats).to.be.null;
                        expect(record.SeasonStats).to.equal(
                            '00111100010100100000010100110101'
                        );
                    ...
                    it('Baker Mayfield', () => {
                        ...
                        expect(record.GameStats).to.be.null;
                        expect(record.SeasonStats).to.equal(
                            '00111100010100100000010100110110'
                        );
                    ...
                    it('Baker Mayfield - table2 field (First Name)', () => {
                        ...
                        expect(field.unformattedValue).to.eql(
                            Buffer.from([
                                0x42, 0x61, 0x6b, 0x65, 0x72, 0x00, 0x00, 0x00,
                ...
                file.save(filePaths.saveTest.m20).then(() => {
                    ...
                    file2.on('ready', () => {
                        ...
                        table2.readRecords().then(() => {
                            ...
                            expect(
                                table2.records[0].fieldsArray[0].unformattedValue.getBits(
                                    0,
                                    32
                                )
                            ).to.eql(0x7fffffff);

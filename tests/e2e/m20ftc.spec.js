    ...
    describe('post-open tests', () => {
                ...
                describe('reads records', () => {
                    ...
                    describe('first record', () => {
                        ...
                        it('access values directly from record', () => {
                            ...
                            expect(record.Entries).to.equal(
                                '00000110011110000000000000000000'
                            );
                            expect(record.Name).to.equal('AwardType');
                        });

                        it('getValueByKey()', () => {
                            expect(record.getValueByKey('Entries')).to.equal(
                                '00000110011110000000000000000000'
                            );
                            expect(record.getValueByKey('Name')).to.equal(
                                'AwardType'
                            );
                        });
                        ...
                        it('getFieldByKey()', () => {
                            ...
                            expect(entriesField.value).to.equal(
                                '00000110011110000000000000000000'
                            );
                            expect(
                                entriesField.unformattedValue.getBits(0, 32)
                            ).to.equal(0x6780000);
                            ...
                            expect(
                                nameField.unformattedValue.getBits(32, 32)
                            ).to.equal(0);
                        });

                        it('table2 field', () => {
                            let nameTable2Field =
                                record.getFieldByKey('Name').secondTableField;
                            ...
                            expect(nameTable2Field.unformattedValue).to.eql(
                                Buffer.from([
                                    0x41, 0x77, 0x61, 0x72, 0x64, 0x54, 0x79,
                                    0x70, 0x65, 0x00
                                ])
                            );
                        });
                    ...
                    describe('second record', () => {
                        ...
                        it('access values directly from record', () => {
                            expect(record).to.not.be.undefined;
                            expect(record.Entries).to.equal(
                                '00000110011001100000000000000000'
                            );
                        ...
                        it('table2 field', () => {
                            ...
                            expect(nameTable2Field.maxLength).to.equal(44);
                            expect(nameTable2Field.value).to.equal(
                                'City Personality'
                            );
                            expect(nameTable2Field.unformattedValue).to.eql(
                                Buffer.from([
                                    0x43, 0x69, 0x74, 0x79, 0x20, 0x50, 0x65,
                                    0x72, 0x73, 0x6f, 0x6e, 0x61, 0x6c, 0x69,
                                    0x74, 0x79, 0x00
                                ])
                            );
                        });
            ...
            describe('AbilityProgressionTunable[]', () => {
                ...
                describe('reads records', () => {
                    ...
                    describe('first record', () => {
                        ...
                        it('access values directly from record', () => {
                            expect(record).to.not.be.undefined;
                            expect(record.AbilityProgressionTunable0).to.equal(
                                '00000100001000100000000000000000'
                            );
                            expect(record.AbilityProgressionTunable3).to.equal(
                                '00000100001000100000000000000011'
                            );
                        });
                    });
                    ...
                    describe('tenth record', () => {
                        ...
                        it('access values directly from record', () => {
                            expect(record).to.not.be.undefined;
                            expect(record.AbilityProgressionTunable0).to.equal(
                                '00000100001000100000000001100110'
                            );
                            expect(record.AbilityProgressionTunable17).to.equal(
                                '00000100001000100000000001101100'
                            );
                        });
                    });
                });
                ...
                describe('Stadium', () => {
                    ...
                    describe('reads records', () => {
                        ...
                        describe('tenth record', () => {
                            ...
                            it('access values directly from record', () => {
                                expect(record).to.not.be.undefined;
                                expect(record.STADIUM_ASSETNAME).to.equal(
                                    'sphere'
                                );
        ...
        describe('can save changes', () => {
            ...
            it('can save with a change to integer fields', (done) => {
                ...
                table.readRecords().then(() => {
                    ...
                    file.save(filePaths.saveTest.ftc).then(() => {
                        let file2 = new FranchiseFile(filePaths.saveTest.ftc);
                        file2.on('ready', () => {
                            let table2 = file.getTableByName(
                                'PlayerAbilityAdjustment'
                            );
                            table2.readRecords().then(() => {
                                expect(table2.records[15].MaxPercent).to.equal(
                                    50
                                );
                                expect(table2.records[15].MinPercent).to.equal(
                                    25
                                );
                                done();
                            });
                ...
                table.readRecords().then(() => {
                    ...
                    file.save(filePaths.saveTest.ftc).then(() => {
                        ...
                        file2.on('ready', () => {
                            ...
                            table2.readRecords().then(() => {
                                expect(table2.records[0].Name).to.equal(
                                    'AwardType'
                                );
                                expect(table2.records[5].Name).to.equal(
                                    'Reasons To cut'
                                );
                                expect(table2.records[7].Name).to.equal(
                                    'DraftTestTestPosition'
                                );
            ...
            it('record field values remain correct after modifiying a table2 value and saving', (done) => {
                ...
                table.readRecords().then(() => {
                    table.records[7].Name = 'DraftTestTestPosition';
                    expect(table.records[7].Name).to.equal(
                        'DraftTestTestPosition'
                    );

  ...
  describe('post-open tests', () => {
    ...
    describe('correctly parses tables', () => {
      ...
      describe('PopularityComponentTable', () => {
        ...
        it('can get a binary reference to a record', () => {
          ...
          expect(reference).to.eql('00100000101100000000000000000110');
        });
      ...
      describe('Player[] table', () => {
        ...
        describe('reads records', () => {
          ...
          it('reads records correctly', () => {
            ...
            expect(record.Player0).to.eql('00100001000001100000011101000110');
            expect(record.Player1).to.eql('00100001000001100000101011111011');
            expect(record.Player2).to.eql('00100001000001100000000000000000');
            expect(record.hexData.slice(0, 10)).to.eql(Buffer.from([33, 6, 7, 70, 33, 6, 10, 251, 33, 6]));
          });
            ...
            file.save(filePathToSave).then(() => {
              ...
              file2.on('ready', () => {
                ...
                table2.readRecords().then(() => {
                  ...
                  expect(table2.records[0].Player1).to.eql('00100001000001100000101011111011');
                  expect(table2.records[0].Player2).to.eql('00100001000001100000000000000000');
                  done();
                });
              });
            });
              ...
              file.save(filePathToSave).then(() => {
                ...
                file2.on('ready', () => {
                  ...
                  table2.readRecords().then(() => {
                    ...
                    expect(table2.records[0].Player0).to.eql('00100001000001100000011000100110');
                    expect(table2.records[0].Player1).to.eql('00100001000001100000001000101101');
    ...
    describe('Player table', () => {
      ...
      it('has correct schema', () => {
        ...
        expect(table.schema.attributes[104].name).to.equal('MinInjuryDuration');
      });

      describe('reads records that are passed in', () => {
        beforeEach((done) => {
          table.readRecords(['MorphHead', 'FirstName', 'LastName', 'Position', 'TRAIT_BIGHITTER', 'MetaMorph_GutBase', 'SeasonStats', 'InjuryType', 'TRAIT_COVER_BALL']).then(() => {
            done();
          });
        });
        ...
        describe('records have expected values', () => {
          ...
          it('Marcus Maye', () => {
            ...
            expect(record.GameStats).to.be.null;
            expect(record.SeasonStats).to.equal('00101111001111000000010111110000');
          ...
          it('Baker Mayfield', () => {
            ...
            expect(record.GameStats).to.be.null;
            expect(record.SeasonStats).to.equal('00101111001111000000010111110001');
        ...
        it('"autoUnempty: true" will un-empty the row if an empty player row is edited', () => {
          ...
          expect(record.MorphHead).to.equal('00000000000000000000000000000000'); // should clear out the first 4 bytes
        });
      });
    ...
    describe('can follow references', () => {
      it('can follow reference data correctly', () => {
        ...
        const expectedResult = file.getTableById(playerTableId).records[2811];
    ...
    describe('Tweet', () => {
      ...
      describe('can replace raw data', () => {
        ...
        it('re-read records from the same data', async () => {
          ...
          expect(table.records[1].Tweet).to.equal('As of this week, there\'s NFL action to watch EVERY SINGLE WEEK until February! :fire: ');
          expect(table.records[1].AuthorName).to.equal('NFL @NFL');
        });

        it('can replace with modified data', async () => {
          const modifiedData = fs.readFileSync(path.join(__dirname, '../data/table-import/24_TweetTableModified.dat'));

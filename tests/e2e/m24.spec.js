...
const filePaths = {
    ...
    uncompressed: {
        ...
        m24: 'tests/data/24UNCOMPRESS.frt'
    },
    saveTest: {
        m21: 'tests/data/CAREER-TESTSAVE-21',
        m23: 'tests/data/CAREER-TESTSAVE-23',
        m24: 'tests/data/CAREER-TESTSAVE-24'
    }
};

const playerTableId = 4227;
const playerArrayTableIdToTest = 6025;

const filePathToUse = filePaths.compressed.m24;
const filePathToSave = filePaths.saveTest.m24;

let file;

describe('Madden 24 end to end tests', function () {
    this.timeout(7000);

    describe('open files', () => {
        it('can open a M24 compressed file', () => {
            file = new FranchiseFile(filePathToUse);
        });

        it('can open a M24 uncompressed file', () => {
            file = new FranchiseFile(filePaths.uncompressed.m24);
        });
        ...
        it('fires the `ready` event when the file is done processing', (done) => {
            ...
            file.on('ready', () => {
                ...
                expect(file.gameYear).to.equal(24);
                ...
                expect(file.schemaList.meta.major).to.equal(155);
                expect(file.schemaList.meta.minor).to.equal(8);
                expect(file.schemaList.path).to.contain('M24_155_8.gz');
    ...
    describe('post-open tests', () => {
        ...
        describe('correctly parses tables', () => {
            ...
            describe('PopularityComponentTable', () => {
                ...
                it('parses expected attribute values', () => {
                    ...
                    expect(table.offset).to.equal(2565060);
                });

                it('parsed expected header', () => {
                    expect(table.header).to.not.be.undefined;
                    expect(table.header.tableId).to.equal(4184);
                ...
                it('can get a binary reference to a record', () => {
                    ...
                    expect(reference).to.eql(
                        '00100000101100000000000000000110'
                    );
                });
            ...
            describe('Player[] table', () => {
                ...
                it('parses expected attribute values', () => {
                    ...
                    expect(table.hexData).to.not.be.undefined;
                    expect(table.readRecords).to.exist;
                    expect(table.offset).to.equal(11133858);
                });
                ...
                it('parsed expected header', () => {
                    ...
                    expect(table.header.table1Length).to.equal(60);
                });
                                ...
                                file2.on('ready', () => {
                                    ...
                                    table2.readRecords().then(() => {
                                        ...
                                        expect(
                                            ...
                                            '00100001000001100000011000100110'
                                        );
                                        expect(
                                            table2.records[0].Player1
                                        ).to.eql(
                                            '00100001000001100000001000101101'
                                        );
                                        expect(
        ...
        describe('Player table', () => {
            ...
            it('parses expected attribute values', () => {
                ...
                expect(table.hexData).to.not.be.undefined;
                expect(table.readRecords).to.exist;
                expect(table.offset).to.equal(7695186);
            });
            ...
            it('parsed expected header', () => {
                ...
                expect(table.header.record1Size).to.equal(364);
                expect(table.header.headerSize).to.equal(1576);
                expect(table.header.hasSecondTable).to.be.true;
                expect(table.header.table1StartIndex).to.equal(1576);
                expect(table.header.table1Length).to.equal(1442816);
                expect(table.header.table2StartIndex).to.equal(1443016);
                expect(table.header.table2Length).to.equal(415800);
            });

            it('has correct schema', () => {
                expect(table.schema).to.not.be.undefined;
                expect(table.schema.attributes.length).to.equal(336);
                expect(table.schema.attributes[0].name).to.equal(
                    'IsUserControlled'
                );
                expect(table.schema.attributes[1].name).to.equal(
            ...
            describe('reads records that are passed in', () => {
                ...
                it('has expected offset table', () => {
                    ...
                    expect(table.offsetTable.length).to.equal(330);

                    let offset0 = table.offsetTable[0];
                    expect(offset0.name).to.equal('MorphHead');
                    expect(offset0.isReference).to.be.true;
                    expect(offset0.originalIndex).to.equal(105);
                    expect(offset0.index).to.equal(105);
                    ...
                    expect(offset7.name).to.equal('SeasonStats');
                    expect(offset7.isReference).to.be.true;
                    expect(offset7.originalIndex).to.equal(267);
                    expect(offset7.index).to.equal(267);
                    expect(offset7.offset).to.equal(224);
                    expect(offset7.indexOffset).to.equal(224);
                    expect(offset7.length).to.equal(32);

                    let runningStyleOffset = table.offsetTable[316];
                    expect(runningStyleOffset.name).to.equal(
                        'RunningStyleRating'
                    );
                    expect(runningStyleOffset.isReference).to.be.false;
                    expect(runningStyleOffset.originalIndex).to.equal(264);
                    expect(runningStyleOffset.index).to.equal(264);
                    expect(runningStyleOffset.offset).to.equal(2855);
                    expect(runningStyleOffset.indexOffset).to.equal(2868);
                ...
                describe('records have expected values', () => {
                    it('first record', () => {
                        ...
                        expect(record.MetaMorph_GutBase).to.equal(0);
                    ...
                    it('Marcus Maye', () => {
                        ...
                        expect(record.GameStats).to.be.null;
                        expect(record.SeasonStats).to.equal(
                            '00101111001111000000010111110000'
                        );
                    ...
                    it('Baker Mayfield', () => {
                        ...
                        expect(record.GameStats).to.be.null;
                        expect(record.SeasonStats).to.equal(
                            '00101111001111000000010111110001'
                        );
                    ...
                    it('Baker Mayfield - table2 field (First Name)', () => {
                        ...
                        expect(field.index).to.equal(192360);
                ...
                it('"autoUnempty: false" will not un-empty the row if an empty player row is edited', () => {
                    ...
                    expect(record.isEmpty).to.be.true;
                    const valueBefore = record.MorphHead;

                    record.FirstName = 'NotEmpty';
                    expect(record.isEmpty).to.be.true;
                    expect(record.MorphHead).to.equal(valueBefore); // should not clear out the first 4 bytes
                });
                ...
                it('"autoUnempty: true" will un-empty the row if an empty player row is edited', () => {
                    ...
                    expect(record.MorphHead).to.equal(
                        '00000000000000000000000000000000'
                    ...
                    table
                        ...
                        .then(() => {
                            expect(table.records[0].GameStats).to.not.be
                                .undefined;
                            expect(table.loadedOffsets.length).to.equal(10);
                            done();
                        })
                        .catch((err) => {
                            done(err);
        ...
        describe('EndofSeasonResigningStartReaction', () => {
            ...
            it('parses expected attribute values', () => {
                ...
                expect(table.hexData).to.not.be.undefined;
                expect(table.readRecords).to.exist;
                expect(table.offset).to.equal(11789327);
            });

            it('parsed expected header', () => {
                expect(table.header).to.not.be.undefined;
                expect(table.header.tableId).to.equal(4594);
                expect(table.header.data1RecordCount).to.equal(1);
        ...
        describe('can follow references', () => {
            it('can follow reference data correctly', () => {
                ...
                const expectedResult =
                    file.getTableById(playerTableId).records[2811];
        ...
        describe('Tweet', () => {
            ...
            describe('can replace raw data', () => {
                ...
                it('re-read records from the same data', async () => {
                    ...
                    expect(table.emptyRecords.size).to.equal(99);

                    expect(table.records[1].TweetHash).to.equal(333535142);
                    expect(table.records[1].Tweet).to.equal(
                        "As of this week, there's NFL action to watch EVERY SINGLE WEEK until February! :fire: "
                    );
                    expect(table.records[1].AuthorName).to.equal('NFL @NFL');
                });
                ...
                it('can replace with modified data', async () => {
                    const modifiedData = fs.readFileSync(
                        path.join(
                            __dirname,
                            '../data/table-import/24_TweetTableModified.dat'
                        )
                    );
                    ...
                    expect(table.header.tableId).to.equal(4296);
                ...
                it('modified data is saved properly', async () => {
                    ...
                    const table = file2.getTableById(4296);
        ...
        describe('can get references to a specific record', () => {
            ...
            it('expected result - Team', () => {
                const references = file.getReferencesToRecord(6030, 0);
                ...
                expect(references[4].tableId).to.eql(4621);
        ...
        describe('TeamNeedEvaluation', () => {
            ...
            it('does not zero out the first 32 bits since changed field is part of first 32 bytes', () => {
                ...
                expect(
                    table.records[834].data.readUInt32BE(0)
                ).to.be.greaterThan(0);
            });
        ...
        describe('Coach', () => {
            ...
            it("when the first 4 bytes are zeroed out, the first column's value changes as well", () => {
                ...
                expect(table.records[138].CharacterVisuals).to.equal(
                    '00000000000000000000000000000000'
                );
            });
        ...
        describe('CharacterVisuals (table3)', () => {
            ...
            it('handles un-empty scenario (not manually emptying first)', (done) => {
                ...
                let offsetTableEntry =
                    table.records[recordIndex]._fields.RawData.offset;
                const oldOffset1 = table.records[
                    recordIndex
                ]._fields.RawData.unformattedValue.getBits(
                    offsetTableEntry.offset,
                    offsetTableEntry.length
                );
                const oldOffset2 =
                    table.records[recordIndex]._fields.RawData.thirdTableField
                        .offset;

                table.records[recordIndex].Overflow =
                    '00000000000000000000000000000000';
                ...
                expect(table.records[recordIndex].RawData).to.eql(
                ...
                ).to.not.equal(oldOffset1);
                expect(
                    table.records[recordIndex]._fields.RawData.thirdTableField
                        .offset
                ).to.not.equal(oldOffset2);
                ...
                file.save(filePathToSave).then(() => {
                    ...
                    file2.on('ready', async () => {
                        ...
                        offsetTableEntry =
                            table2.records[recordIndex]._fields.RawData.offset;

                        expect(table2.records[recordIndex].isEmpty).to.be.false;
                        expect(table2.records[recordIndex].RawData).to.eql(
                            JSON.stringify(newData)
                        );
                        expect(
                            table2.records[
                        ...
                        expect(
                            table2.records[recordIndex]._fields.RawData
                                .thirdTableField.offset
                        ).to.not.equal(oldOffset2);
                        done();
                    });
                });
            ...
            describe('handles scenario where data exists between the table3 size & data', () => {
                ...
                before((done) => {
                    file3 = new FranchiseFile(
                        'tests/data/CAREER-24Table3TDB2Data'
                    );
                    file3.on('ready', () => {
                ...
                it('can read the data', () => {
                    expect(table.records[43].RawData.length).to.be.greaterThan(
                        0
                    );
                    let parsedData = JSON.parse(table.records[43].RawData);
                    expect(parsedData.assetName).to.eql('AlexanderJaire_13109');
                    expect(
                        table.records[43]._fields.RawData.thirdTableField.unformattedValue.readUInt8(
                            2
                        )
                    ).to.eql(7);

                    const decompressedData = zlib.gunzipSync(
                        table.records[43]._fields.RawData.thirdTableField.unformattedValue.subarray(
                            3
                        )
                    );

                    const convertedData = JSON.stringify(
                        readChviRecord(decompressedData)
                    );
                ...
                it('can set the data', (done) => {
                    ...
                    file3.save(filePathToSave).then(() => {
                        ...
                        file2.on('ready', async () => {
                            ...
                            expect(table2.records[43].RawData).to.eql(
                                JSON.stringify(newData)
                            );
                            done();
                        });

...
const zlibSpy = {
    inflateSync: sinon.spy((data) => {
        return zlib.inflateSync(data);
    }),
    gunzipSync: sinon.spy((data) => {
...
describe('Franchise File unit tests', () => {
    ...
    describe("can identify a file's type", () => {
        it('Decompressed M19', () => {
            const file = new FranchiseFile(
                filePaths.decompressed.m19,
                franchiseFileOptions
            );
        ...
        it('Compressed FTC', () => {
            const file = new FranchiseFile(
                filePaths.ftc.tuningCompressed,
                franchiseFileOptions
            );
            expect(file.type).to.eql({
        ...
        it('Decompressed FTC', () => {
            const file = new FranchiseFile(
                filePaths.ftc.tuningDecompressed,
                franchiseFileOptions
            );
            expect(file.type).to.eql({
                format: Constants.FORMAT.FRANCHISE_COMMON,
    ...
    describe('can uncompress a compressed file', () => {
        it('Decompressed M19', () => {
            ...
            common.hashCompare(
                file.unpackedFileContents,
                files.decompressed.m19
            );
        ...
        it('Compressed M19', () => {
            ...
            expect(zlibSpy.inflateSync.args[0][0].slice(0, 2)).to.eql(
        ...
        it('Compressed FTC', () => {
            ...
            common.hashCompare(
                file.packedFileContents,
                files.ftc.tuningCompressed
            );
            expect(file.unpackedFileContents).to.not.be.undefined;
            expect(zlibSpy.inflateSync.calledOnce).to.be.true;
            expect(zlibSpy.inflateSync.args[0][0].slice(0, 2)).to.eql(

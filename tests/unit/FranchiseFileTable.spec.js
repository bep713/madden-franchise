...
const expectedOffsetTables = {
    player: JSON.parse(
        fs.readFileSync('tests/data/offset-tables/M24_playerOffsetTable.json')
    ),
    tunableData: JSON.parse(
        fs.readFileSync(
            'tests/data/offset-tables/M24_SeasonScheduleManager.SeasonScheduleTunableDataOffsetTable.json'
        )
    )
};
...
const schemaData = {
    player: JSON.parse(
        fs.readFileSync('tests/data/test-schemas/M24/PlayerSchema.json')
    ),
    tunableData: JSON.parse(
        fs.readFileSync(
            'tests/data/test-schemas/M24/SeasonScheduleManager.SeasonScheduleTunableDataSchema.json'
        )
    )
};
...
const tableData = {
    ...
    tunableData: fs.readFileSync(
        'tests/data/tables/M24_SeasonScheduleTunableData_Table.dat'
    )
};
...
describe('FranchiseFileTable unit tests', () => {
    ...
    beforeEach(async () => {
        ...
        FranchiseFileTable = (await import('../../src/FranchiseFileTable.js'))
            .default;
    ...
    describe('offset table', () => {
        ...
        describe('with schema', () => {
            it('schedule tunable data', async () => {
                ...
                expect(
                    JSON.parse(JSON.stringify(tunableDataTable.offsetTable))
                ).to.eql(expectedOffsetTables.tunableData);
            });
            ...
            it('player', async () => {
                ...
                expect(
                    JSON.parse(JSON.stringify(playerTable.offsetTable))
                ).to.eql(expectedOffsetTables.player);
            });
            ...
            it('if schema is incorrect - will auto-override with generic fields', async () => {
                ...
                assertGeneratedSchema(
                    playerTable.offsetTable,
                    expectedOffsetTables.player
                );
            });
        ...
        describe('without schema', () => {
            it('schedule tunable data', async () => {
                ...
                await tunableDataTable.readRecords();
                assertGeneratedSchema(
                    tunableDataTable.offsetTable,
                    expectedOffsetTables.tunableData
                );
            });

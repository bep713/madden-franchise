...
describe('FranchiseSchema unit tests', () => {
    it('can load a gzip file', (done) => {
        const schema = new FranchiseSchema(
            'tests/data/test-schemas/M20_360_1.gz'
        ).once('schemas:done', () => {
            ...
            const relativeAppt = schema.schemas.find((schema) => {
                return schema.name === 'Scheduler.RelativeApptDateTime';
            });
    ...
    it('can load a xml file', (done) => {
        const schema = new FranchiseSchema(
            'tests/data/test-schemas/schema-19.xml'
        ).once('schemas:done', () => {
    ...
    it('can load a ftx file', (done) => {
        const schema = new FranchiseSchema(
            path.join(__dirname, '../data/test-schemas/367_1.FTX')
        ).once('schemas:done', () => {

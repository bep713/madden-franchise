...
describe('schemaGeneratorV2 unit tests', () => {
    ...
    before(async () => {
        schemaRoot = await generateSchemaV2({
            fileMap: { main: SCHEMA_PATHS.m22.xml }
        });
    });
...
describe('schemaGeneratorV2 IncludeFile dependency tests', () => {
    it('parses includes before main file', async () => {
        const mainPath = path.join(
            __dirname,
            '../data/test-schemas/include-mock/main.FTX'
        );
        const includePath = path.join(
            __dirname,
            '../data/test-schemas/include-mock/Football-Schemas.FTX'
        );
        const fileMap = { main: mainPath, 'Football-Schemas': includePath };
        const result = await generateSchemaV2({ fileMap });
        expect(result.schemas.some((s) => s.name === 'IncludedSchema')).to.be
            .true;

        const includedEnum = result.schemaMap.TestEnum.attributes.find(
            (s) => s.name === 'Test'
        );

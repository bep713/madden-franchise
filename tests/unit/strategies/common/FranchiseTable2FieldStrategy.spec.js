...
describe('Franchise Table2 Field Strategy', () => {
    describe('get initial unformatted value', () => {
        it('returns expected value', () => {
            ...
            const buffer = Buffer.from([0x43, 0x6F, 0x61, 0x63, 0x68, 0x20, 0x54, 0x72, 0x61, 0x69, 0x74, 0x00, 0x00, 0x00, 0x00, 0x70, 0x71]);
        ...
        it('second table field index > 0', () => {
            ...
            const buffer = Buffer.from([0x43, 0x6F, 0x61, 0x63, 0x68, 0x20, 0x54, 0x72, 0x61, 0x69, 0x74, 0x00, 0x10, 0x00, 0x00, 0x70]);
    ...
    describe('set unformatted value from formatted value', () => {
        it('normal case', () => {
            ...
            expect(result).to.eql(Buffer.from([0x68, 0x65, 0x6C, 0x6C, 0x6F, 0x00, 0x00, 0x00, 0x00, 0x00]));
        });

        it('empty value', () => {
            const result = FranchiseTable2FieldStrategy.setUnformattedValueFromFormatted('', 10);
            expect(result).to.eql(Buffer.from([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]));
        });
